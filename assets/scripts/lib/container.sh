...
renderCompose() {
  ...
  if ! envsubst '$BASE_PATH $SECRETS_PATH $DEPLOYMENTS_PATH $SOCKETS_PATH $CONTAINER_PATH $MOUNTS_PATH $SUBNET_CORE $SUBNET_MODULE $SUBNET_GATEWAY $CORE_DB_PW $CORE_DB_ROOT_PW $CORE_ID $CORE_NAME $GATEWAY_PORT $CORE_USR_PW $NGINX_IMG $MYSQLDB_IMG $KRATOS_IMG $AUTH_SERVICE_IMG $MODULE_MANAGER_IMG $MODULE_MANAGER_MIGRATION_IMG $SECRET_MANAGER_IMG $WEB_UI_IMG $HOST_DIR_REPOSITORY_PATH $CTR_RESTART_POLICY' < $container_path/docker-compose.yml.template > $container_path/docker-compose.yml
  then
    exit 1
  fi
}
